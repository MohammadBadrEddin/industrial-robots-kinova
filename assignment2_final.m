...
for seg = 1:size(waypoints,2)-1
    ...
    for step = 1:N
        ...
        hold on;
        axis([-0.8 0.8 -0.8 0.8 -0.1 1.0]);
        view([60, 20]);

        % ---- compute Jacobian and manipulability -----------------
        J    = geometricJacobian(gen3, q_cur, eeName);
        % geometricJacobian returns [angular; linear] velocity rows,
        % i.e. rows 1-3 = rotational, rows 4-6 = translational.
        Jv   = J(4:6, :);          % translational part (bottom 3 rows)

        % SVD: Jv = U * S * V'
        % U(:,1) = direction of LARGEST singular value
        %        = direction of HIGHEST translational capability = vm
        [U, S, ~] = svd(Jv);
        vm = U(:, 1);              % manipulability direction

        % Yoshikawa measure: mu = product of singular values of Jv
        % (equivalent to sqrt(det(Jv*Jv')), but numerically robust
        %  near singular configurations - avoids underflow issues)
        % mu is large when the robot is far from singularity
        % mu → 0 when the robot is near a singular configuration
